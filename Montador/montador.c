...
void DetectarLabels(void)
{
    ...
    while (Look != EOF)
    {
        ...
        else
        {
            ...
            /* ### MEU CODIGO INICIO ### */
            case BEQ_CODE : // 3 argumentos e 2 linhas
            case ADDI_CODE:
                parser_SkipUntil(',');
                parser_SkipUntil(',');
                parser_SkipUntilEnd();
		    ...
		    free(str_tmp2);
                ...
                */
                ...
                case BEQ_CODE :
                    ...
                    str_tmp1 = ConverteRegistrador(val1);
                    str_tmp2 = ConverteRegistrador(val2);
                    str_tmp3 = NumPBinString(val3);

                    // Escreve instrução em binario
                    sprintf(str_msg,"%s%s%s%s",CMP,str_tmp1,str_tmp2, COND_EQ);

                    // Escreve endereço na proxima linha
                    parser_Write_Inst(str_msg,end_cnt);
                    end_cnt += 1;
                    sprintf(str_msg,"%s",str_tmp3);
                    parser_Write_Inst(str_msg,end_cnt);
                    end_cnt += 1;
                ...
                case ADDI_CODE :
                    ...
                    // Escreve instrução em binario
                    sprintf(str_msg,"%s%s%s0000",ADDI,str_tmp1,str_tmp2);

                    // Escreve número na proxima linha
                    parser_Write_Inst(str_msg,end_cnt);
                    end_cnt += 1;
                    sprintf(str_msg,"%s",str_tmp3);
                    parser_Write_Inst(str_msg,end_cnt);
...
int BuscaInstrucao(char * nome)
{
    ...
    */
    ...
    // MEU CODIGO INI
    else if (strcmp(str_tmp,ADDI_STR) == 0)
    {
        return ADDI_CODE;
    }
    // MEU CODIGO FIM
